...
class UniversalProvider extends BaseProvider {
    ...
    async getMangaDetails(manga) {
        return await BrowserManager.withPage(async (page) => {
            ...
            try {
                // Wait for the actual manga details to be added to DOM, or fallback to networkidle.
                await page.waitForLoadState('networkidle', { timeout: 15000 });
            } catch (e) {
                console.log('[Universal] Network idle timeout reached, proceeding with available content.');
            }

            // Retry fetching content in case of a late CloudFlare redirect
            let html;
            
            // Check for Turnstile Cloudflare iframe
            try {
                const turnstileFrame = await page.waitForSelector('iframe[src*="turnstile"]', { timeout: 5000 });
                if(turnstileFrame) {
                    console.log('[Universal] Cloudflare Turnstile detected. Attempting to click/solve...');
                    await page.waitForTimeout(3000); // Give challenge time to configure
                    // Try to click center of the frame
                    const box = await turnstileFrame.boundingBox();
                    if(box) {
                        await page.mouse.click(box.x + box.width / 2, box.y + box.height / 2);
                        console.log('[Universal] Turnstile clicked. If a puzzle appears, PLEASE SOLVE IT now! You have 30 seconds...');
                        
                        // Wait for navigation or networkidle
                        await Promise.race([
                ...
                // No Turnstile found or error clicking, proceed normally.
            }
            ...
            for (let i = 0; i < 3; i++) {
                try {
                    ...
                    console.log(`[Universal] Navigation occurred during content extraction, retrying... (${i + 1}/3)`);
            ...
            const chaptersList = [];
              ...
              // --- RULE 1: SAME-MANGA LINEAGE (Slug Extraction) ---
              ...
              targetScope.each((_, el) => {
                  ...
                  // --- RULE 4: MULTI-SIGNAL CHAPTER DETECTION ---
              ...
              // --- RULE 1 & 7: SAME-MANGA LINEAGE & CONTAMINATION FILTER ---
              ...
              // --- RULE 3 & 7: URL STRUCTURE CONSISTENCY (Dominant Pattern) ---
              ...
              // --- RULE 6: VALIDATION & DEDUPLICATION ---
              ...
              // --- RULE 8: CONFIDENCE RULE (FAIL-SAFE) ---
              const finalApproved = finalChapters.length < 3 ? [] : finalChapters;

              // --- ORDER: Sort by chapter number descending ---
            ...
            return {
                ...
                chapters: uniqueChapters.sort((a, b) => b.number - a.number)
            };
        });
    }
        ...
        return await BrowserManager.withPage(async (page) => {
            ...
            // Aggressively block ad-networks and extra chapters
            await page.route('**/*', (route) => {
                const url = route.request().url().toLowerCase();
                const adKeywords = [
                    'google-analytics', 'doubleclick', 'adonxs', 'popads', 'zergnet', 
                    'mgid', 'taboola', 'outbrain', 'exoclick', 'adsterra', 'ad-delivery',
                    'clickadu', 'propellerads', 'revenuehits', 'infolinks', 'popcash'
                ];
                ...
                // Block infinite scroll loading the NEXT chapter via AJAX
                if (url.includes('admin-ajax.php') && (url.includes('action=manga_') || url.includes('action=z_do_ajax'))) {
                    return route.abort();
                }
            ...
            page.on('request', (request) => {
                ...
                if (url.match(/\.(webp|jpg|png|jpeg)/i) &&
                   ...
                   !url.includes('banner') && !url.includes('bg') &&
                   !url.includes('index.jpg') && !url.includes('background') &&
                   !url.includes('discord') && !url.includes('sponsor') &&
                   !url.includes('promo') && !url.includes('donate') && 
                   !url.includes('tiktok') && !url.includes('paypal') && 
                   !url.includes('ko-fi') && !url.includes('credit') && 
            ...
            // DESTROY next chapter fetchers in the DOM before scrolling down
            await page.evaluate(() => {
            ...
            for (let i = 0; i < 20; i++) {
                ...
                await page.waitForTimeout(400); // let image lazily populate
                const newHeight = await page.evaluate(() => document.body.scrollHeight);
                if (newHeight === previousHeight && i > 6) {
                    break;
                }
            ...
            // Strategy 2: Class/ID-based specific extraction
            if (imageUrls.size === 0) {
                $('img').each((_, el) => {
                    const src = extractSrc(el);
                    ...
                    if (classStr.includes('chapter') || classStr.includes('page') ||
                        classStr.includes('reader') || classStr.includes('img-') ||
                        idStr.includes('image') || parentStr.includes('page') || parentStr.includes('reader')) {
            ...
            if (imageUrls.size <= 3 && imageRequests.size > 3) {
                imageUrls.clear();
                for (const req of imageRequests) {
                    imageUrls.add(req);
                }
            } else if (imageUrls.size === 0 && imageRequests.size > 0) {        
                for (const req of imageRequests) {
                    imageUrls.add(req);
                }
            }

            // FILTER: Strip out cross-contamination from adjacent chapters
            const targetMatched = chapter.url.match(/(?:\bchapter|\bch|\bepisode|\bep|\bc)[-_=]?(\d+[\.]?\d*)/i);
            const targetChapNum = targetMatched ? parseFloat(targetMatched[1]) : null;

            let strictArray = Array.from(imageUrls);
            if (targetChapNum) {
                strictArray = strictArray.filter(url => {
                    const matchedUrls = url.match(/(?:\/chapter|\/ch-|\/episode|\/ep-)(\d+[\.]?\d*)\//i);
                    if (matchedUrls) {
                        const imgUrlChapNum = parseFloat(matchedUrls[1]);
                        if (imgUrlChapNum > 0 && imgUrlChapNum !== targetChapNum && Math.abs(imgUrlChapNum - targetChapNum) >= 1) {
                            return false;
                        }
                    }
                    return true;
            ...
            if (strictArray.length === 0) {
                throw new Error(`Universal Provider found zero images for ${chapter.url}`);
            }

            return this.normalizeResult(strictArray);
        });
    }
}
