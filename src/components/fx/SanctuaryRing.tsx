...
const SanctuaryRing: React.FC<{ theme: Theme; isPaused?: boolean }> = ({ theme, isPaused = false }) => {
    ...
    return (
        <div className="fixed inset-0 pointer-events-none z-0 flex items-center justify-center opacity-20 transition-opacity duration-700" aria-hidden="true">
            {/* motion-safe: the ring holds still under prefers-reduced-motion. */}
            <svg viewBox="0 0 500 500" className={`w-[150vh] h-[150vh] ${!isPaused ? 'motion-safe:animate-[spin_120s_linear_infinite]' : ''}`}>
