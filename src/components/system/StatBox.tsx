    ...
    <SystemFrame variant="brackets" theme={theme} className={`${theme.isDark ? 'bg-black/40' : 'bg-white/40'} ${className} transition-colors duration-700`}>
        <div className="flex flex-col items-center justify-center h-full gap-1.5 py-3">
            <div className="relative w-14 h-14 flex items-center justify-center mb-0.5">
                ...
                <Icon className={`w-7 h-7 ${color} transition-colors duration-700`} />
            </div>
            <div className={`text-xl font-black font-mono ${theme.headingText} tracking-wide transition-colors duration-700`}>{value}</div>
            <div className={`text-[10px] font-mono ${theme.highlightText} font-black uppercase tracking-[0.2em] opacity-100 transition-colors duration-700`}>{label}</div>
        </div>
    </SystemFrame>
);
