...
const TowerHUD: React.FC<TowerHUDProps> = ({ items, theme, onActivate, isFocused = false, selectedFloorIndex = 0, itemsPerFloor = 5 }) => {
    // 1. Calculate Stats
    ...
    // Derived Rank (Always based on global title count — stays consistent with main dashboard)
    ...
    return (
        <div className={`absolute inset-0 z-30 pointer-events-none pt-24 md:pt-24 px-2 md:px-8 pb-6 md:pb-8 flex flex-row items-center md:items-start justify-between overflow-hidden gap-1 md:gap-6 ${isFocused ? 'hidden md:flex' : 'flex'}`}>
            {/* LEFT: STATS PANEL */}
            <div className={`flex flex-col gap-2 md:gap-6 pointer-events-auto w-[32%] sm:w-[40%] md:w-[45%] max-w-xs transition-transform duration-700 ease-in-out ${isFocused ? 'translate-y-0' : 'translate-y-0'}`}>
                ...
                {!isFocused && (
                    <div className="flex flex-col gap-4 md:gap-6">
                        <div className="relative overflow-hidden group">
                            ...
                            <div className={`border ${theme.border} p-1.5 md:p-3 flex items-center justify-between gap-2 ${theme.isDark ? 'bg-black/60' : 'bg-white/60'} backdrop-blur-md shadow-lg`}>
                                <div className="flex flex-col min-w-0 flex-1">
                                    ...
                                    <div
                                        className={`font-black font-manifold italic tracking-tight drop-shadow-sm text-transparent bg-clip-text bg-gradient-to-r ${theme.gradient} leading-none truncate`}
                                        style={{ fontSize: 'clamp(0.8rem, 2.5vw, 1.75rem)' }}
                                    >
                                        {rawRank.label}
                                    </div>
                                </div>
                                <Crown size={isFocused ? 12 : 16} className={`shrink-0 ${theme.highlightText} drop-shadow-lg opacity-80 md:w-8 md:h-8`} />
                            </div>
                        </div>
                        ...
                        <div className={`border ${theme.borderSubtle} p-3 ${theme.isDark ? 'bg-black/40' : 'bg-white/40'} backdrop-blur-md`}>
                            <div className={`flex justify-between text-[8px] md:text-xs font-mono font-bold ${theme.highlightText} mb-2`}>
                                <span className="truncate">EXP ACQUIRED</span>
                                <span className="whitespace-nowrap">{globalTotalChapters.toLocaleString()} PTS</span>
                            </div>
                            <div className={`h-1.5 w-full ${theme.isDark ? 'bg-gray-800' : 'bg-gray-200'} `}>
                                <div className={`h-full bg-gradient-to-r ${theme.gradient} progress-bloom transition-all duration-700`} style={{ width: `${Math.min(100, (globalTotalChapters % 1000) / 10)}%`, color: theme.id === 'LIGHT' ? '#0ea5e9' : '#fbbf24' }} />
                            </div>
            ...
            <div className={`pointer-events-auto flex flex-col items-end gap-2 md:gap-3 w-[32%] sm:w-[40%] md:w-72 max-w-xs transition-transform duration-700 ease-in-out ${isFocused ? 'translate-y-[0]' : 'translate-y-0'}`}>
                            ...
                            >
                                ...
                                <div className="flex-1 min-w-0 flex flex-col justify-center text-left">
                                    ...
                                    <div className="flex items-center gap-2 text-[10px] font-mono text-gray-400 justify-start">
                                        <span className={`font-rajdhani font-bold tabular-nums ${theme.isDark ? 'text-amber-300' : 'text-cyan-600'} text-xs`}>CH: {item.currentChapter} / {item.totalChapters || '?'}</span>
                                    </div>
                                </div>
                            </div>
                        ))}
                                    ...
                                    return (
                                        <div key={cls}>
                                            ...
                                            <div className={`h-1 w-full ${theme.isDark ? 'bg-gray-800' : 'bg-gray-200'} `}>
                                                <div className={`h-full transition-all duration-700 bg-gradient-to-r ${theme.gradient} progress-bloom `} style={{ width: `${Math.min(100, pct)}%`, color: theme.id === 'LIGHT' ? '#0ea5e9' : '#fbbf24' }} />
                                            </div>
                                        </div>
                                    );
                                });
                ...
                <div className="w-full">
                    ...
                    {displayItems.filter(i => i.status === 'ACTIVE' && (i.totalChapters ?? 0) > 0).length > 0 && (
                        <div className={`border ${theme.borderSubtle} ${theme.isDark ? 'bg-black/50' : 'bg-white/50'} backdrop-blur-md p-3 flex flex-col gap-3`}>
                            {[...displayItems]
                                .filter(i => i.status === 'ACTIVE' && (i.totalChapters ?? 0) > 0)
                                .slice(0, 3)
                                .map(item => {
                                    const pct = Math.round((item.currentChapter / (item.totalChapters ?? 1)) * 100);
                                    return (
                                        <div key={item._id}>
                                            <div className="flex justify-between mb-1">
                                            ...
                                            <div className={`h-1.5 w-full ${theme.isDark ? 'bg-gray-800' : 'bg-gray-200'} `}>
                                                <div className={`h-full transition-all duration-700 ${pct >= 80 ? `bg-gradient-to-r ${theme.gradient}` : `bg-gradient-to-r ${theme.gradient}`} progress-bloom `} style={{ width: `${Math.min(100, pct)}%`, color: theme.id === 'LIGHT' ? '#0ea5e9' : '#fbbf24' }} />
                                            </div>
                                        </div>
                                    );
                                })
